    ...
    public static Intent getPickImageIntent(Context context, String chooserTitle) {
        ...
        if (!appManifestContainsPermission(context, Manifest.permission.CAMERA) || hasCameraAccess(context)) {
            ...
            Uri uri = FileProvider.getUriForFile(context, context.getApplicationContext().getPackageName()
                    + ".provider", getTemporalFile(context));


           /* // Check for the freshest data.
            if (Build.VERSION.SDK_INT == Build.VERSION_CODES.KITKAT) {
                final int takeFlags = takePhotoIntent.getFlags() & (Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
